...
export function useFetchForecast(location){
    ...
    useEffect(() => {
        if (location !== undefined) {
            setLoading(true)
            fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${location.lat}&lon=${location.lon}&exclude=hourly,minutely,alerts&units=metric&appid=9bad5acbe78e5cb42955c85fdf9cc36b`)
                .then((response) => {
                    if (response.ok) {
                        setError(null)
